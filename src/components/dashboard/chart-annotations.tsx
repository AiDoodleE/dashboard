...
type ChartAnnotationsProps = {
  width: number
  height: number
  onAddAnnotation?: (annotation: Omit<Annotation, 'id' | 'date'>) => void
...
export function ChartAnnotations({
  ...
  const [newAnnotation, setNewAnnotation] = useState<Pick<Annotation, 'title' | 'note' | 'type'>>({ 
    title: '', 
    note: '',
    type: 'info'
  })
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    {['info', 'warning', 'success', 'custom'].map((type) => (
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation()
                          setNewAnnotation(prev => ({ ...prev, type: type as any }))
                        }}
                      >
                        {type[0].toUpperCase()}
                      </button>
