        ...
        <div className="flex items-center justify-between">
          ...
          <Button 
            ...
            onClick={() => {
              ...
              // Title
              doc.setFontSize(16);
              doc.text("AI Insights Report", 14, y);
              y += 10;

              insights.forEach((insight) => {
                // Set title for each insight
                doc.setFontSize(13);
                ...
                doc.text(`Type: ${insight.type} | Impact: ${insight.impact}`, 14, y);
                y += 6;

                // Add description (with word wrap)
                const splitDescription = doc.splitTextToSize(insight.description, 180);
                doc.text(splitDescription, 14, y);
                y += 6 * splitDescription.length;

                // Add suggested action if present
                if (insight.suggestedAction) {
                  doc.setFont(undefined, 'bold');
                  doc.text("Suggested Action:", 14, y);
                  y += 6;
                  doc.setFont(undefined, 'normal');
                  const splitAction = doc.splitTextToSize(insight.suggestedAction, 180);
                  doc.text(splitAction, 14, y);
                  y += 6 * splitAction.length;
                }
                ...
                if (y > 270 && insights.indexOf(insight) < insights.length - 1) {
                  doc.addPage();
                  y = 20;
                }
              });
